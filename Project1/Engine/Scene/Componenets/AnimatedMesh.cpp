...
AnimatedMesh::AnimatedMesh(std::vector<Vertex> verticies, std::vector<uint32_t> indicies, std::vector<VkCommandBuffer>& commandBuffers, VkDevice& logicalDevice, std::vector<VkImage>& swapChainImages, VkExtent2D& swapChainExtent, VkPhysicalDevice& physicalDevice, VkCommandPool& commandPool, VkQueue& graphicsQueue, std::string& texturePath, VkDescriptorSetLayout layout):
...
void AnimatedMesh::createTextureImageViews()
{
...
void AnimatedMesh::updateUniformBuffer(uint32_t currentImage)
{
	animUbo.proj = glm::perspective(glm::radians(45.0f), rm_swapChainExtent.width / (float)rm_swapChainExtent.height, 0.1f, 100.0f);
	animUbo.proj[1][1] *= -1;

	void* data;
	vkMapMemory(rm_logicalDevice, m_uniformBuffersMemory[currentImage], 0, sizeof(animUbo), 0, &data);
	memcpy(data, &animUbo, sizeof(animUbo));
	vkUnmapMemory(rm_logicalDevice, m_uniformBuffersMemory[currentImage]);
}

void AnimatedMesh::createTextureImages(std::string texturePath)
{
	// TODO : need to decide if loading in all texture data is better done outside of this class eg: loading in models.
	TextureReturnVals vals = ResourceManager::getSingleton().loadTextureFile(texturePath);
	...
	memcpy(data, vals.pixels.data(), static_cast<size_t>(imageSize));
...
void AnimatedMesh::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
...
void AnimatedMesh::createTextureSampler()
{
...
void AnimatedMesh::createDescriptorSet()
{
	...
	for (size_t i = 0; i < rm_swapChainImages.size(); i++) {
		...
		bufferInfo.offset = 0;
		bufferInfo.range = sizeof(Anim_UBO);
...
void AnimatedMesh::createVertexBuffer()
{
...
void AnimatedMesh::createIndexBuffer()
{
...
void AnimatedMesh::createUniformBuffers()
{
	VkDeviceSize bufferSize = sizeof(Anim_UBO);
